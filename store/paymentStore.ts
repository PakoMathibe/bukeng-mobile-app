...
import { Order, PaymentIntent, Instalment, Transaction } from '@/types/transaction';
...
interface PaymentState {
  ...
  updateInstalment: (instalmentId: string, updates: Partial<Instalment>) => void;
    ...
    updateInstalment: (instalmentId, updates) =>
      set((state) => ({
        instalments: state.instalments.map((instalment) =>
          instalment.id === instalmentId ? { ...instalment, ...updates } : instalment
        ),
        orders: state.orders.map((order) => ({
          ...order,
